...
namespace TapTap.Friends.Internal {
    ...
    public class TDSFriendCore {
        ...
        private static string GenerateShareLink(string roleName, string funcName, string id, Dictionary<string, object> queries) {
            ...
            Dictionary<string, object> queryParams = new Dictionary<string, object>();
            queryParams.Add("p", HttpUtility.UrlEncode(Convert.ToBase64String(Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(ps)))));

            // 开发者自定义透传参数
            if (queries?.Count > 0) {
                foreach (KeyValuePair<string, object> kv in queries) {
                    queryParams.Add(HttpUtility.UrlEncode(kv.Key), HttpUtility.UrlEncode(kv.Value as string));
                }
            }
